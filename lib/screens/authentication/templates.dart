...
import 'package:intelliTAG/utility/global.dart';
import 'package:intelliTAG/utility/network.dart';
...
class _RegisterTemplateState extends State<RegisterTemplate> {
  ...
  List<Widget> _interestsTags = new List<Widget>();
  List<Widget> _skillsTags = new List<Widget>();
  ...
  GlobalKey<AutoCompleteTextFieldState<String>> keyInterests = new GlobalKey();
  GlobalKey<AutoCompleteTextFieldState<String>> keySkills = new GlobalKey();
  ...
  void executeRegistration() async {
    ...
    FirebaseAuth.instance
      ...
      Firestore.instance.collection("Users").document(userId).setData({
        ...
        "last_name": _lastName,
        "events_attended": {},
        "tags": {"interests": _interests, "skills": _skills},
        "uid": userId,
      ...
      body: Stack(
        children: <Widget>[
            Center(
              child: Container(
                width: screenWidth * 0.8,
                child: (!isUpdatingTags)?enterUserInfoScreen():enterTagsScreen()
              ),
            ),
        ...
        Card(
          child: ListTile(
            ...
            trailing: RaisedButton(
              ...
              onPressed: () {
                String text = interestsController.text;
                if (text != "") {
                  _interests.add(text);
                  _interestsTags.add(InputChip(
                    label: Text(text.toString()),
                    onDeleted: () {
                      _interests.remove(text);
                      setState(() {
                        _interests;
                        _interestsTags;
                      });
